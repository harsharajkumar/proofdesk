...
# Bump this when the HTML/SVG rendering pipeline changes in a way that should
# invalidate previously generated pretex caches.
PRETEX_CACHE_VERSION = 'renderer-20260323-1'
...
def parse_svg_number(text, default=0.0):
    "Parse a numeric SVG attribute value, ignoring any unit suffix."
    if text is None:
        return default
    match = re.match(
        r'\s*(-?(?:\d+(?:\.\d*)?|\.\d+)(?:[eE][+-]?\d+)?)', str(text))
    if not match:
        return default
...
class HTMLDoc:
    ...
    def __init__(self, html_file, preamble, tmp_dir, args):
        ...
        self.basename = b64_hash(
            PRETEX_CACHE_VERSION + '\0' + self.html_data
        )
    ...
    def is_cached_svg(self):
        return (
            self.svg_cache is not None
            and self._is_valid_cache(self.svg_cache)
        )

    @property
    def is_cached_html(self):
        return self._is_valid_cache(self.html_cache)
    ...
    def _is_valid_cache(self, path):
        ...
        try:
            ...
            return False
        # If a cached file still contains placeholders or raw LaTeX script
        # tags, it came from an incomplete pretex pass and should not be
        # reused.
        return (
            b'pretex-placeholder' not in data
            and b'text/x-latex-' not in data
        )
    ...
    def make_latex(self):
        ...
        self.contents_hash = b64_hash(
            PRETEX_CACHE_VERSION + '\0' + contents
        )
            ...
            for line in fobj.readlines():
                ...
                if match:
                    ...
                    if match:
                        ...
                        # Keep display pages in sequence even when tounicode.py
                        # cannot rewrite the legacy Poppler bbox metadata.
                        typ = 'display'
                        bbox_missing = True
                        left = 0
                        top = 0
    ...
    def write_cache(self, style, fonts, svgs):
        ...
        with open(self.svg_cache, 'wb') as fobj:
            fobj.write(self._serialize_html(cache))

    def _serialize_html(self, elt):
        "Serialize HTML while restoring SVG attribute casing that lxml.html lowercases."
        content = html.tostring(
            elt, include_meta_content_type=True, encoding='utf-8')
        return content.replace(b'viewbox=', b'viewBox=')
    ...
    def use_cached_svg(self):
        ...
        with open(self.svg_cache, 'rb') as fobj:
            cache = html.fromstring(fobj.read().decode('utf-8'))
        style = cache[0].text
        fonts = cache[1].text
        ...
        contents = self._serialize_html(self.dom)
...
'''.format(name=name, data=b64encode(data).decode('ascii'))
        ...
        content = self._serialize_html(self.dom)
        with open(self.output_file, 'wb') as outf:
            outf.write(content)
        with open(self.html_cache, 'wb') as outf:
            outf.write(content)
        ...
        for page_num, page_extents in enumerate(self.pages_extents):
            try:
                with open(self.svg_file(page_num), 'rb') as fobj:
                    svg_bytes = fobj.read()
                    svg_bytes = re.sub(
                        br'^\s*<\?xml[^>]*>\s*', b'', svg_bytes, count=1)
                    svg = html.fromstring(svg_bytes.decode('utf-8'))
            except Exception as _psvg_err:
                print('  process_svgs: skipping page {} - {}'.format(page_num, _psvg_err))
                continue
            # Remove extra attrs from <svg>
            original_viewbox = svg.attrib.get('viewBox', '')
            original_width = parse_svg_number(svg.attrib.get('width'))
            original_height = parse_svg_number(svg.attrib.get('height'))
            ...
            if page_extents['display']:
                if page_extents.get('bbox_missing'):
                    raw_viewbox = [parse_svg_number(x) for x in
                                   re.split(r'[\s,]+', original_viewbox.strip())
                                   if x]
                    if len(raw_viewbox) != 4:
                        raw_viewbox = [0.0, 0.0, original_width, original_height]
                    pt_scale = 72/96
                    if units_in_pt:
                        viewbox = [pt_scale * val for val in raw_viewbox]
                    else:
                        viewbox = raw_viewbox
                    svg.attrib['viewBox'] = '{} {} {} {}'.format(
                        smart_float(viewbox[0]),
                        smart_float(viewbox[1]),
                        smart_float(viewbox[2]),
                        smart_float(viewbox[3])
                    )
                    svg.attrib['height'] = '{}em'.format(
                        smart_float((raw_viewbox[3] * pt_scale) /
                                    page_extents['fontsize']))
                else:
            ...
            if page_extents['display']:
                ...
                svg = div
                # Add tags
                if not page_extents.get('bbox_missing'):
...
def main():
    ...
    #tmpdir = os.path.realpath('./tmp')
        ...
        proc = Popen(['python3', TOUNICODE, '--outdir', sfd_dir] + pdf_files,
