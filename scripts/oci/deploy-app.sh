#!/usr/bin/env bash
# Deploys the Proofdesk app onto an already-provisioned OCI instance.
...
#
# OCI Ubuntu 24.04 default SSH user is "ubuntu".
# OCI Oracle Linux default SSH user is "opc".
# Override with: SSH_USER=opc ./scripts/oci/deploy-app.sh
set -euo pipefail
...
SSH="ssh $SSH_OPTS ${SSH_USER}@${PUBLIC_IP}"

# OCI instances may have firewalld enabled (Oracle Linux) or iptables rules that block
# ports 80/443 even when the security list allows them. Flush those if present.
OCI_FIREWALL_FLUSH='
  ...
  # Ubuntu iptables may also have a default REJECT rule for Docker
...
scp $SSH_OPTS \
  "$ENV_FILE" \
  "${SSH_USER}@${PUBLIC_IP}:/home/${SSH_USER}/.env.production"

scp $SSH_OPTS \
  "$SCRIPT_DIR/bootstrap-instance.sh" \
  "${SSH_USER}@${PUBLIC_IP}:/home/${SSH_USER}/bootstrap-instance.sh"

# ── Install Docker ────────────────────────────────────────────────────────────
log "Running Docker bootstrap (needs sudo — may take ~2 min) ..."
$SSH "sudo bash /home/${SSH_USER}/bootstrap-instance.sh"

# ── Open OCI host firewall ports ──────────────────────────────────────────────
log "Opening host firewall ports 80/443 ..."
$SSH "bash -c '$OCI_FIREWALL_FLUSH'"

# ── Add swap (small instances only have 1-6 GB RAM; Node builds need more) ───
...
# ── Copy env file ─────────────────────────────────────────────────────────────
log "Installing .env ..."
$SSH "cp /home/${SSH_USER}/.env.production /opt/proofdesk/.env"

# ── Pre-build PreTeXt builder image ──────────────────────────────────────────
log "Building mra-pretext-builder image on host (skipped if already present) ..."
$SSH bash << 'REMOTE'
  set -euo pipefail
  if sudo docker image inspect mra-pretext-builder >/dev/null 2>&1; then
    ...
    echo "[remote] Building mra-pretext-builder (this takes 15-20 min on first run) ..."
